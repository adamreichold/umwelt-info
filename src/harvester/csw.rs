...
pub async fn harvest(dir: &Dir, client: &Client, source: &Source) -> Result<()> {
    ...
    let max_records = source.batch_size.unwrap_or(100);
...
async fn fetch_datasets(
    ...
    let response: GetRecordsResponse = from_str(&text)?;
...
async fn write_dataset(dir: &Dir, source: &Source, record: SummaryRecord) -> Result<()> {
    let dataset = Dataset {
        ...
        source_url: source
            .url
            .join(&format!("catalog.search#/metadata/{}", record.identifier))?,
    };
